# Content Plan — Month 1 (batch & schedule)

8 posts across 4 weeks (2/week), built on the 4 pillars from the acquisition plan:
**Deal of the week · Business spotlight · Lompoc local · Milestone.**

Batch-write these once, schedule them (Meta Business Suite lets you queue FB + IG together),
then your only weekly job is replying to comments and dropping spotlights into local FB
groups. Spotlights are timed to follow onboarding — **publish a spotlight the same week a
business goes live** so the tag-and-reshare loop fires while they're excited.

**Channels:** Facebook page + Instagram (queue together) + paste into local FB groups
("Lompoc Community Board," "Lompoc Buy/Sell/Trade," "What's Happening in Lompoc") where group
rules allow promo. Facebook is the priority — that's where Lompoc's community actually is.

**Hashtag block (reuse, trim to ~8 on IG):**
`#LompocLocals #Lompoc #LompocCA #OldTownLompoc #ShopLocalLompoc #SupportLocal #CentralCoast #805 #VandenbergSFB`

> Anything in **[brackets]** is a swap-in — replace with the actual business that went live
> that week. The spotlight order below assumes you land the three social-ready Tier-1 spots
> (South Side Coffee, Old Town Kitchen, Lompoc Barber Lounge) first, since they'll reshare.

---

## Week 1 — Launch

### Post 1.1 — Announcement (the "we exist" post)
- **Pillar:** Announcement / Milestone
- **Image:** Simple branded graphic — "Lompoc Locals" logo + tagline over an Old Town / flower
  fields photo.
- **Caption:**
  > Lompoc, meet your new deals feed. 🌼
  > We're rounding up the best coupons, specials, and happy hours from *local* Lompoc spots —
  > all in one place, free to browse.
  > 👉 Check it out: www.lompoclocals.com
  > Local business owner? Get your spot listed free — DM us. We're setting up the first 25
  > businesses ourselves. 🤝
- **Notes:** Pin this to the top of your FB page. Post to all three groups.

### Post 1.2 — Lompoc local (engagement bait)
- **Pillar:** Lompoc local
- **Image:** Fun question graphic or a photo of a local breakfast burrito.
- **Caption:**
  > Settle a debate for us, Lompoc 👇
  > **Best breakfast burrito in town — who's got it?**
  > Drop the name. We're hungry and we're taking notes. 🌯
- **Notes:** Pure reach play — no link, just comments. Reply to every answer; it trains the
  algorithm and starts relationships with the businesses people name.

---

## Week 2 — First spotlight + first deal

### Post 2.1 — Business spotlight #1: [South Side Coffee Co.]
- **Pillar:** Business spotlight
- **Image:** Photo of their storefront / a latte. Ask them for one when you onboard.
- **Caption:**
  > 👋 Meet **[South Side Coffee Co.]** on H Street — and one of our very first Lompoc Locals
  > founding businesses. ☕
  > Stop in, say hi, and grab their deal on the app.
  > 📍 [105 S H St] · @[southsidecoffeeco_lompoc]
- **Notes:** **Tag their IG/FB so they reshare to their followers** — this is the whole point.
  Confirm the handle before posting.

### Post 2.2 — Deal of the week: [South Side Coffee Co.]
- **Pillar:** Deal of the week
- **Image:** Their first deal as a graphic (e.g. "$1 OFF any espresso drink").
- **Caption:**
  > 🔥 This week's Lompoc deal: **$1 off any espresso drink at [South Side Coffee Co.]**
  > Show the deal at the counter. That's it.
  > 👉 Grab it: www.lompoclocals.com/en/biz/[south-side-coffee-co]
- **Notes:** Pulls straight from the first-deal draft. Post to groups Thursday/Friday (weekend
  coffee runs).

---

## Week 3 — Spotlight + community

### Post 3.1 — Business spotlight #2: [Old Town Kitchen & Bar]
- **Pillar:** Business spotlight
- **Image:** Their food / dining room.
- **Caption:**
  > New on Lompoc Locals: **[Old Town Kitchen & Bar]** 🍽️
  > Old Town's spot for [a good meal and a drink] — now with a deal for locals on the app.
  > 📍 [319 E Ocean Ave] · @[oldtownkitchen_805]
- **Notes:** Tag them. If they have a signature dish, name it — specifics get shares.

### Post 3.2 — Lompoc local (poll)
- **Pillar:** Lompoc local
- **Image:** "This or that" graphic.
- **Caption:**
  > Friday plans, Lompoc edition. Where are you headed? 🍻
  > 🅰️ Taproom & wings
  > 🅱️ Tacos & a margarita
  > Comment your pick — best answer gets featured next week.
- **Notes:** Whoever you spotlight next can be teased here. Keeps a content thread going.

---

## Week 4 — Spotlight + milestone

### Post 4.1 — Business spotlight #3: [Lompoc Barber Lounge]
- **Pillar:** Business spotlight
- **Image:** Their shop / a fresh cut.
- **Caption:**
  > Fresh cut, local spot. 💈 **[Lompoc Barber Lounge]** is now on Lompoc Locals.
  > New clients — there's a deal waiting for you on the app.
  > 📍 [137 N H St] · @[lompocbarberlounge]
- **Notes:** Tag them. Barbers' clients love repping their shop — good reshare odds.

### Post 4.2 — Milestone (social proof for the *next* pitch)
- **Pillar:** Milestone
- **Image:** Grid/collage of the logos or storefronts of everyone who's joined.
- **Caption:**
  > [10] Lompoc businesses are now on Lompoc Locals — and we're just getting started. 🌼
  > Coffee, food, drinks, cuts, and more, all with local deals in one place.
  > 👉 Browse them: www.lompoclocals.com
  > Run a Lompoc spot? It's free to join — DM us. 🤝
- **Notes:** Swap [10] for the real count. **This post is also a sales tool** — screenshot it
  and show it to the next business you pitch ("here's who's already on").

---

## After Month 1

- Keep the rhythm: ~1 spotlight + 1 deal/community post per week, always fed by who you
  onboarded that week. No inventing content.
- Once ~15 businesses are live and the feed looks active, that's the **Phase 2 trigger** —
  flip toward locals: push the weekly email digest and lean harder into the FB groups.
- Save your best-performing post format and repeat it. Let the data pick your style.
