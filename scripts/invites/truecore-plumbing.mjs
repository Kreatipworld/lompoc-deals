#!/usr/bin/env node
// Growth invite for TrueCore Plumbing LLC (biz 679, created Sep 19 2026).
// Signed "The Lompoc Locals team" (no owner name), no "free" framing, one CTA.
//   node scripts/invites/truecore-plumbing.mjs                     # dry run
//   SEND=1 PREVIEW=1 node scripts/invites/truecore-plumbing.mjs    # proof → hello@
//   TO=owner@example.com SEND=1 node scripts/invites/truecore-plumbing.mjs                      # send now
//   TO=owner@example.com AT="2026-09-21T21:00:00.000Z" SEND=1 node scripts/invites/truecore-plumbing.mjs  # scheduled (Resend holds it)
...
const TO = process.env.TO || "cristianl805@truecoreplumbing.com"  // from their website
const biz = { name: "TrueCore Plumbing LLC", slug: "truecore-plumbing" }
  ...
  <div style="padding:28px 24px; border:1px solid #eee; border-top:none; border-radius:0 0 12px 12px;">
    ...
    <div style="background:#F7F3E9; border:1px solid #E9DFC2; border-radius:12px; padding:20px 22px; margin:0 0 22px;">
      ...
      <ul style="color:#444; line-height:1.7; margin:0 0 14px; padding-left:20px;">
        <li style="margin-bottom:6px;"><strong>Show up first</strong> when a neighbor searches for a plumber, a water heater, or a burst pipe in Lompoc.</li>
        <li style="margin-bottom:6px;"><strong>24/7 means nothing if they can't find you at 2am</strong> &mdash; your page puts tap-to-call at the top.</li>
        <li style="margin-bottom:6px;"><strong>Post an offer</strong> &mdash; it lands in the deals feed <em>and</em> the Monday email, town-wide.</li>
      ...
      <p style="margin:0;"><a href="${claimUrl}" style="display:inline-block; background:${P}; color:#ffffff; padding:13px 24px; border-radius:8px; text-decoration:none; font-weight:600;">Claim TrueCore Plumbing LLC &amp; become a member</a></p>
    </div>

    <p style="color:#444; line-height:1.6; margin:0 0 16px;"><strong>Week one, once you're in:</strong> a Member Spotlight video built from your own job photos on Instagram, TikTok, and Facebook, and your first offer in the Monday email. Just reply to this email and we'll set it up with you.</p>
    ...
    <div style="margin-top:26px; padding-top:18px; border-top:1px solid #eee; text-align:center;">
      ...
      <div style="font-size:11px; color:#aaa; margin-top:12px; line-height:1.5;">You're getting this because TrueCore Plumbing LLC is listed on Lompoc Locals. <a href="${unsubUrl(TO || "hello@lompoclocals.com")}" style="color:#aaa; text-decoration:underline;">Unsubscribe</a> &mdash; or reply &ldquo;unsubscribe&rdquo;.<br>${POSTAL}</div>
    </div>
  </div>
</div>`
...
const subject = `TrueCore Plumbing LLC is on Lompoc Locals — your page and the claim link`
