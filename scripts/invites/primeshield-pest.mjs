#!/usr/bin/env node
// Growth invite for Primeshield Pest Solutions (biz 677, created Sep 18 2026).
// Signed "The Lompoc Locals team" (no owner name), no "free" framing, one CTA.
//   node scripts/invites/primeshield-pest.mjs                     # dry run
//   SEND=1 PREVIEW=1 node scripts/invites/primeshield-pest.mjs    # proof → hello@
//   TO=owner@example.com SEND=1 node scripts/invites/primeshield-pest.mjs   # real send (only on explicit "send")
...
const biz = { name: "Primeshield Pest Solutions", slug: "primeshield-pest-solutions" }
...
<div style="font-family: system-ui, -apple-system, sans-serif; max-width: 560px; margin: 0 auto; background:#ffffff;">
  ...
  <div style="padding:28px 24px; border:1px solid #eee; border-top:none; border-radius:0 0 12px 12px;">
    <h1 style="font-size:23px; margin:0 0 10px; color:#1a1a1a; font-weight:800; letter-spacing:-0.01em;">Primeshield Pest Solutions is on Lompoc Locals.</h1>
    <div style="height:3px; width:52px; background:${Y}; border-radius:2px; margin:0 0 18px;"></div>
    <p style="color:#444; line-height:1.6; margin:0 0 14px;">We built you a page on <strong>Lompoc Locals</strong> &mdash; the #1 local hub in Lompoc, where neighbors find local businesses, read local news, follow Friday night football, and see what's happening in town each week. Your page is live now &mdash; <a href="${profileUrl}" style="color:${P}; font-weight:700;">take a look</a>. Send us your logo, phone number, service area and the pests you treat, and we'll finish it the same day.</p>
    <p style="color:#444; line-height:1.6; margin:0 0 18px;">A few facts: neighbors opened <strong>${F.views30d} business pages</strong> on Lompoc Locals in the last 30 days, and every Monday morning our email &mdash; local news, this week's events, and members' deals &mdash; lands in local inboxes across town. Local trades like <strong>Terrones Plumbing</strong>, <strong>Wm Rieck Plumbing</strong>, and <strong>Clark Builders</strong> are already members, with the partner badge on the map, a spot in the homepage member rail, and their deals in the Monday email.</p>
    ...
    <div style="background:#F7F3E9; border:1px solid #E9DFC2; border-radius:12px; padding:20px 22px; margin:0 0 22px;">
      ...
      <ul style="color:#444; line-height:1.7; margin:0 0 14px; padding-left:20px;">
        <li style="margin-bottom:6px;"><strong>Show up first</strong> when a neighbor searches for pest control, ants, termites, or rodents in Lompoc.</li>
        ...
        <li style="margin-bottom:6px;"><strong>Be reachable</strong> &mdash; tap-to-call, your website, and your service area on one page.</li>
      ...
      <p style="margin:0;"><a href="${claimUrl}" style="display:inline-block; background:${P}; color:#ffffff; padding:13px 24px; border-radius:8px; text-decoration:none; font-weight:600;">Claim Primeshield Pest Solutions &amp; become a member</a></p>
    </div>

    <p style="color:#444; line-height:1.6; margin:0 0 16px;"><strong>Week one, once you're in:</strong> a Member Spotlight video built from your own photos and logo on Instagram, TikTok, and Facebook, and your first offer in the Monday email. Just reply to this email and we'll set it up with you.</p>
    ...
    <div style="margin-top:26px; padding-top:18px; border-top:1px solid #eee; text-align:center;">
      ...
      <div style="font-size:11px; color:#aaa; margin-top:12px; line-height:1.5;">You're getting this because Primeshield Pest Solutions is listed on Lompoc Locals. <a href="${unsubUrl(TO || "hello@lompoclocals.com")}" style="color:#aaa; text-decoration:underline;">Unsubscribe</a> &mdash; or reply &ldquo;unsubscribe&rdquo;.<br>${POSTAL}</div>
    </div>
  </div>
</div>`
...
const subject = `Primeshield Pest Solutions is on Lompoc Locals — your page and the claim link`
