#!/usr/bin/env node
// Growth invite for Focus Physical Therapy (created Sep 23 2026, Vandenberg Village).
// Signed "The Lompoc Locals team" (no owner name), no "free" framing, one CTA.
//   node scripts/invites/focus-physical-therapy.mjs                     # dry run
//   SEND=1 PREVIEW=1 node scripts/invites/focus-physical-therapy.mjs    # proof → hello@
//   TO=owner@example.com SEND=1 node scripts/invites/focus-physical-therapy.mjs                      # send now
//   TO=owner@example.com AT="2026-09-21T21:00:00.000Z" SEND=1 node scripts/invites/focus-physical-therapy.mjs  # scheduled (Resend holds it)
...
const TO = process.env.TO || "focuspt21@gmail.com"  // the owner's address on their website (office: focuspt200@yahoo.com)
const biz = { name: "Focus Physical Therapy", slug: "focus-physical-therapy" }
  ...
  <div style="padding:28px 24px; border:1px solid #eee; border-top:none; border-radius:0 0 12px 12px;">
    ...
    <div style="background:#F7F3E9; border:1px solid #E9DFC2; border-radius:12px; padding:20px 22px; margin:0 0 22px;">
      ...
      <ul style="color:#444; line-height:1.7; margin:0 0 14px; padding-left:20px;">
        <li style="margin-bottom:6px;"><strong>Show up first</strong> when a neighbor searches for physical therapy, lymphedema care, or pelvic health in Lompoc.</li>
        <li style="margin-bottom:6px;"><strong>Tap-to-call and your hours at the top</strong> &mdash; a new patient reaches the front desk in one tap.</li>
        <li style="margin-bottom:6px;"><strong>Post a class or an announcement</strong> &mdash; a Spinning Babies&reg; date or a new service lands in the feed <em>and</em> the Monday email, town-wide.</li>
      ...
      <p style="margin:0;"><a href="${claimUrl}" style="display:inline-block; background:${P}; color:#ffffff; padding:13px 24px; border-radius:8px; text-decoration:none; font-weight:600;">Claim Focus Physical Therapy &amp; become a member</a></p>
    </div>

    <p style="color:#444; line-height:1.6; margin:0 0 16px;"><strong>Week one, once you're in:</strong> a Member Spotlight video built from your own clinic photos on Instagram, TikTok, and Facebook, and your first announcement in the Monday email. Just reply to this email and we'll set it up with you.</p>
    ...
    <div style="margin-top:26px; padding-top:18px; border-top:1px solid #eee; text-align:center;">
      ...
      <div style="font-size:11px; color:#aaa; margin-top:12px; line-height:1.5;">You're getting this because Focus Physical Therapy is listed on Lompoc Locals. <a href="${unsubUrl(TO || "hello@lompoclocals.com")}" style="color:#aaa; text-decoration:underline;">Unsubscribe</a> &mdash; or reply &ldquo;unsubscribe&rdquo;.<br>${POSTAL}</div>
    </div>
  </div>
</div>`
...
const subject = `Focus Physical Therapy is on Lompoc Locals — your page and the claim link`
