 ...
 *   node scripts/render-social-cards.mjs content/social/cards/cards-week2.html content/social/launch-kit/week2
 *
...
const SRC = process.argv[2] || "content/social/cards/cards-week2.html"
const OUT = process.argv[3] || "content/social/launch-kit/week2"
...
for (const { id, size } of cards) {
  ...
  // Attribute selector, not #id: card ids lead with a date and a CSS id selector can't start
  // with a digit.
  const isolated = html.replace(
    "</style>",
    `.card { display:none !important; }
     [id="${id}"] { display:block !important; margin:0 !important; }
