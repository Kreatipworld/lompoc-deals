# Press Pitch — Lompoc Record & SYV News
*Owner: CMO | Channel: PR*
*Created: 2026-04-08 | KPI: 1 feature article, 200 attributed signups*
*Recommended timing: Send after first 50 merchants milestone*

---

## Target Publications

| Publication | Contact approach | Angle |
|------------|-----------------|-------|
| **The Lompoc Record** (lompocrecord.com) | Email news tip or reporter direct | Local tech helping small businesses |
| **Santa Ynez Valley News** (syvnews.com) | Email editor | Expanding to SYV market angle |
| **805 Living** (805living.com) | Editorial pitch | "Best of Central Coast" feature opportunity |
| **KSBY News** (ksby.com) | TV news tip line | Visual story — local business owner interview |

---

## Primary Pitch: The Lompoc Record

**Subject line:** Local story: Lompoc tech startup helping small businesses fight back against big-box

---

Hi [Reporter/Editor name],

I'm reaching out about a local story you might find interesting for your readers.

**The short version:** A free digital platform built for Lompoc is helping local restaurants, salons, and shops post deals directly to Lompoc residents — no algorithm, no out-of-town middleman. It launched this year and already has [X] local businesses and [X] active deals.

**Why it's a Lompoc story:**
The platform is built specifically for Lompoc (there's even a Spanish/English toggle, since we know our community). The founder saw that local businesses were losing foot traffic to chains partly because they had no easy way to reach Lompoc customers digitally. Yelp and Groupon charge steep commissions. This doesn't.

**The human angle:**
[If you have a merchant willing to be quoted — include here: "Maria's Kitchen on [street] has been using it for [time] and says [quote about results]"]

**The numbers:**
- [X] Lompoc businesses listed, [X] active deals
- Free to join for businesses and consumers
- Bilingual (EN/ES) — designed for Lompoc's demographics

**What I'm offering:**
A short interview with the founder, plus photos/screenshots of the platform. Happy to connect you with a merchant for a quote.

Is this something you'd be interested in covering? I can send more details or arrange an interview at your convenience.

Best,
[Contact name]
[Email / phone]
Lompoc Deals
lompoc-deals.vercel.app

---

## Secondary Pitch: SYV News (Expansion Angle)

**Subject line:** Lompoc deals platform eyeing Santa Ynez Valley expansion

---

Hi [Editor],

Following up on a story out of Lompoc that may be relevant to SYV readers.

**Lompoc Deals** (lompoc-deals.vercel.app) is a bilingual platform connecting Santa Barbara County residents with deals from local businesses. It launched in Lompoc and is built to expand into the Santa Ynez Valley and Vandenberg Village.

The pitch: local small businesses in SYV face the same challenge as Lompoc — chains and big-box stores have digital marketing advantages that independent shops don't. This platform is trying to level that.

We'd love to be on your radar as we expand. Happy to arrange an interview or provide more details.

Best,
[Contact name]
Lompoc Deals

---

## TV / Video Pitch: KSBY

**News tip email body:**

> LOCAL BUSINESS STORY: A Lompoc tech platform is helping small businesses compete digitally — free for merchants and consumers. Currently [X] Lompoc businesses, bilingual EN/ES. Visuals available: merchant interviews, deal redemption demo, founder interview. Tip contact: [email/phone]

---

## Talking Points (for founder interviews)

1. **The problem:** Local businesses in Lompoc have no easy, affordable way to reach Lompoc customers digitally. Yelp/Groupon take commissions. Facebook requires ad budget. This is free.

2. **The solution:** Lompoc Deals is a free local deals directory — businesses post deals, residents browse and claim them. No commissions, no national algorithm.

3. **Why Lompoc:** Lompoc is an underserved market. The Hispanic community is 63% of the population, so the platform is fully bilingual. We know this town.

4. **The traction:** [X] businesses, [X] active deals, [X] signups — all organic, no paid ads.

5. **The vision:** Start in Lompoc, expand to Santa Maria, Santa Barbara, Vandenberg Village. Build the local deals network for the Central Coast.

6. **The ask:** Just try it. It's free. Go to lompoc-deals.vercel.app.

---

## Timing Note

**Recommended:** Send this pitch after reaching 50 active merchants (as noted in MARKETING_BACKLOG.md M-012). The story is stronger with a critical mass of businesses — "50 local businesses" is a headline-worthy milestone. "10 businesses" is not.

**Milestone to watch:** When M-003 (Merchant Cold Outreach Sprint) produces enough results to cross 50 merchants, flag this pitch as ready to send.

---

## Follow-up Protocol

1. Send initial pitch
2. If no response in 5 business days, one follow-up: "Following up on my pitch below — happy to provide any additional details or make an intro to a merchant."
3. If no response after follow-up, move to next publication on the list
4. If they respond with interest, prioritize immediately — coordinate with CEO on interview scheduling

---

## Post-Coverage Playbook

If a story runs:
1. Screenshot and post to all social channels immediately
2. Update KPIS.md with traffic spike data
3. Email all existing merchants: "We were featured in [publication] — here's the story"
4. Use press mention in merchant sales collateral ("As featured in The Lompoc Record")
5. Add press badge to homepage (request from CTO)
