...
/**
 * Thursday preview of Saturday's digest, sent to the founder inbox only.
 * The standing rule: no edition reaches subscribers without the founder
 * having seen it two days earlier. Content can shift slightly by Saturday
...
export async function GET(request: Request) {
  ...
  if (!hasMasterDigestContent(content)) {
    await logCronRun("digest-preview", { sent: 0, note: "no content — Saturday will skip" })
    return NextResponse.json({ sent: 0, note: "no content — Saturday will skip" })
  }
  ...
  const banner =
    dealCount === 0
      ? `<div style="background:#fef2f2;border:1px solid #fecaca;color:#b91c1c;padding:12px 16px;border-radius:8px;margin-bottom:16px;font-family:system-ui,sans-serif;font-size:14px;"><strong>Heads up:</strong> zero deals in this edition — nothing was posted in the last 7 days. Nudge a partner or add one before Saturday.</div>`
      : ""
  const { error } = await resend.emails.send({
    from: "Lompoc Locals <hello@lompoclocals.com>",
    to: process.env.NOTIFY_EMAIL ?? "hello@lompoclocals.com",
    subject: "[PREVIEW] Saturday's digest — goes out in 2 days",
    html: banner + html,
  })
