#!/usr/bin/env python3
"""
Coastal Tint — Member Spotlight (Eye on I / Hangar 7 style).
...
SCENES = [
    # id,        start,  dur   — timed to Dylan's read (VO at 0.60): shop/family 0.6–3.2 · name 3.98 · ceramic/heat/UV 5.22–8.1
    #                            · wraps/headlight/detailing 8.66–10.9 · cars/trucks/house/storefront 11.88–14.2 · new in Lompoc… 14.82–17.9 · find them 18.28–19.5
    ("s1-front", 0.00,  5.10),
    ("s3-tint",  4.85,  3.65),
    ("s5-services", 8.25, 3.45),
    ("s6-glass", 11.45, 3.25),
    ("s2-new",   14.45, 2.15),
    ("s4-family", 16.35, 2.05),
    ("s7-end",   18.15, 5.35),
]
TOTAL = 23.50
VO_START = 0.60
...
def end_scene(cid, dur, H, bar):
    ...
    css = common_css(cid, H, bar, "21%", 190) + f"""
      ...
      [data-composition-id="{cid}"] .logocard {{ display: block; width: 56%; margin: 0 auto; background: #fff; border-radius: 40px; padding: 40px 44px; box-shadow: 0 30px 80px rgba(10,6,12,0.5); will-change: transform, opacity; opacity: 0; position: relative; overflow: hidden; }}
      ...
      [data-composition-id="{cid}"] .phone {{ margin-top: 10px; color: {GOLD}; font-weight: 800; }}
  ...
  <div data-composition-id="{cid}" data-width="1080" data-height="{H}" data-duration="{dur:.2f}" style="position: absolute; inset: 0; overflow: hidden; background: {DARK}">
    ...
    <div class="stage" id="{cid}-stage">
      ...
      <div class="col">
        <div id="{cid}-logo" class="logocard"><img src="public/logo.png" alt="Coastal Tint" /><div class="sweep" id="{cid}-sweep"></div></div>
        <span id="{cid}-pill" class="epill">Official Partner</span>
        <span id="{cid}-name" class="ename">Coastal Tint</span>
        <span id="{cid}-addr" class="addr">123 W Ocean Ave, Unit B · Lompoc</span>
        <span id="{cid}-phone" class="addr phone">(820) 228-7620</span>
        <span id="{cid}-menu" class="menu">services · hours · contact</span>
        <span id="{cid}-url" class="url">lompoclocals.com</span>
        <span id="{cid}-rule" class="rule"></span>
      </div>
      <div id="{cid}-credit" class="credit">photos courtesy of Coastal Tint</div>
      <div class="vig"></div>
    </div>
    <div class="grain"></div>
    <div class="cine-bar cine-top"></div><div class="cine-bar cine-bot"></div>
    <script>
      (() => {{
        // BEATS (relative to scene start): LOGO, NAME ("Coastal Tint"), URL ("lompoclocals.com"), END
        ...
        tl.fromTo("#{cid}-phone", {{ autoAlpha: 0, y: 12 }}, {{ autoAlpha: 1, y: 0, duration: 0.4 }}, B.ADDR + 0.12);
        tl.fromTo("#{cid}-menu", {{ autoAlpha: 0, y: 12 }}, {{ autoAlpha: 1, y: 0, duration: 0.4 }}, B.ADDR + 0.25);
...
def scene_html(cid, dur, H, bar):
    wide = H == 1920
    if cid == "s1-front":   # their new shop on Ocean Ave: Mustang inside (landscape → whole over blur)
        return photo_scene(cid, dur, H, bar, "p1.jpg", "whole", ("scale", 1.0, 1.08, "50% 50%"), [],
                           header=("Coastal<br />Tint", "window tint · wraps · Ocean Ave · Lompoc"), whole_y="-50%" if wide else "-40%")
    if cid == "s2-new":     # white M4 front, garage
        return photo_scene(cid, dur, H, bar, "p3.jpg", "cover", ("scale", 1.0, 1.07, "50% 45%"), ["New on Ocean Avenue"], chip_times=[0.40])
    if cid == "s3-tint":    # white M4 side, tinted glass
        return photo_scene(cid, dur, H, bar, "p2.jpg", "cover", ("drift", 3, -3, "50% 45%"), ["Ceramic window tint", "Blocks heat &amp; UV"], chip_times=[0.45, 1.30])
    if cid == "s4-family":  # blue Model Y in the bay
        return photo_scene(cid, dur, H, bar, "p7.jpg", "cover", ("scale", 1.0, 1.07, "45% 50%"), ["Family-owned · Lompoc"], chip_times=[0.30])
    if cid == "s5-services":  # tan Sierra, hard cut to the M4 at dusk (cropped left of the plate)
        return photo_scene(cid, dur, H, bar, "p4.jpg", "cover", ("tilt", 2, -2, "50% 45%"), ["Wraps · Headlight restoration · Detailing"], cut=("p8.jpg", 1.75, "12% 45%"), chip_times=[0.45])
    if cid == "s6-glass":   # from inside a Tiguan, looking out through the tint
        return photo_scene(cid, dur, H, bar, "p5.jpg", "cover", ("scale", 1.0, 1.08, "50% 40%"), ["Cars · Trucks · Homes · Storefronts"], chip_times=[0.45])
    if cid == "s7-end":
...
<html lang="en">
  ...
  <body>
    <!-- COASTAL TINT — Member Spotlight (Eye on I style). {TOTAL:.2f}s. Their own photos, Ken Burns, gold chips, progress line, partner end card.
