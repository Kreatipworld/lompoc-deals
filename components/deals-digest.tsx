...
/**
 * Homepage Weekly Deals Digest — a deals-first section modeled on the Lompoc
 * Locals weekly flyer: featured deal hero + deal grid flanked by sellable
 * sponsor rails. All ad/sponsor CTAs push business membership (/for-businesses).
 */
export async function DealsDigest({ deals }: { deals: DealCardData[] }) {
  ...
  const AdSlot = (
    <Link
      href="/for-businesses"
      className="group flex flex-1 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-primary/30 bg-accent/50 p-5 text-center transition-colors hover:border-primary/60"
    >
      <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary/60">{t("adSpace")}</span>
      <span className="mt-2 font-display text-lg font-bold text-primary">{t("yourAdHere")}</span>
      <span className="mt-2 text-xs leading-relaxed text-muted-foreground">{t("adPitch")}</span>
      <span className="mt-3 inline-flex items-center gap-1 rounded-full bg-success px-4 py-2 text-xs font-bold text-success-foreground">
        {t("claimSpot")} <ArrowRight className="h-3.5 w-3.5" />
      </span>
    </Link>
  )

  const SponsorCard = ({ mono, from, to, name, pitch, cta }: { mono: string; from: string; to: string; name: string; pitch: string; cta: string }) => (
    <div className="flex flex-1 flex-col rounded-2xl border bg-card p-4">
      <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground/70">{t("sponsored")}</span>
      <div className={`mt-2 flex h-16 items-center justify-center rounded-xl bg-gradient-to-br ${from} ${to} text-xl font-extrabold text-white`}>{mono}</div>
      <p className="mt-3 text-sm font-bold leading-tight">{name}</p>
      <p className="mt-1 flex-1 text-xs leading-relaxed text-muted-foreground">{pitch}</p>
      <Link href="/for-businesses" className="mt-3 inline-flex items-center justify-center gap-1 rounded-full bg-success px-3 py-2 text-xs font-bold text-success-foreground">
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14">
      ...
      {/* Stage: left rail · center · right rail */}
      <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-stretch">
        ...
        {/* Right rail */}
        <aside className="flex gap-4 lg:w-44 lg:flex-shrink-0 lg:flex-col">
          {AdSlot}
          <SponsorCard mono="VR" from="from-green-600" to="to-emerald-400" name={t("sponsorRealtyName")} pitch={t("sponsorRealtyPitch")} cta={t("sponsorRealtyCta")} />
        </aside>
      </div>

      {/* Footer sponsor banner */}
      <div className="mt-4">{FeatureBanner}</div>
    </section>
  )
}
